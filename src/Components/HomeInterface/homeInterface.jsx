...
import { Button, ButtonGroup, Card, Dropdown, Image, ListGroup } from 'react-bootstrap';
...
const PatientInterface = ({ userInfo }) => {
    ...
    return (
        <div style={{ marginTop: "50px" }}>
            ...
            <ButtonRow chosenService={chosenService} updateService={updateChosenService} />
            <OldChildServices searchQuery={searchQuery} chosenService={chosenService} userInfo={userInfo} />
        </div>
    )
}
...
const DetailedServiceView = ({ service, handleFormClose, userInfo }) => {
    ...
    const handleDoBooking = async () => {
        if (serviceBooked)
            return;
        try {
            const response = await ApiPostCall("/doBooking", { patientUsername: userInfo.username, serviceId: id })
    ...
    return (
        <div>
            ...
            <h3 className='dsv-title'>{title}</h3><hr />
            ...
            <div className='dsv-caretaker' style={{ marginBottom: "20px" }}>
                ...
                />
                <div>
                    <div style={{ fontWeight: "bold" }}>{capitalizeEachWord(careTakerUsername)}</div>
                    <div style={{ color: "gray" }}>{capitalizeEachWord(careTakerName)}</div>
                </div>
            </div><hr />
    ...
    return (
        <ButtonGroup style={{ width: '100%' }}>
            <Button
                ...
                style={{ width: '50%', borderRadius: '0', padding: "10px", fontWeight: "bold" }}
                onClick={() => setStatus(0)}
            >
                Started
            </Button>
...
const StatusServices = ({ status, userInfo }) => {
    ...
    const getAllServices = async () => {
        try {
            const response = await ApiPostCall("/getUserStatusServices", { status, username: userInfo.username })
    ...
    return (
        <ServiceList services={services} userInfo={userInfo} />
    )
}
...
const OldChildServices = ({ searchQuery, chosenService, userInfo }) => {
    ...
    const getAllServices = async () => {
        try {
            const response = await ApiPostCall("/getServices", { serviceType: chosenService, searchQuery })
...
const ServiceList = ({ services, userInfo }) => {
    ...
    const handleServiceView = (service) => {
        setDetailedService(
            <DetailedServiceView service={service} handleFormClose={onFormClose} userInfo={userInfo} />
        );
        onFormOpen();
    };
